from library.SF_pedestrian import Pedestrian
from library.SF_wall import Wall
import numpy as np
...
def start(env,type_,tm,plot):
    ...
    def update_2(frame):
        ...
        if plot:
            try:
                scat.set_offsets([p.position for p in env.pedestrians]) #왜 여기서 오류가 날까?
            except:
                pass
...
def run_simulation(type_ , target, width, length , plot):
    ...
    for step in tqdm(range(count_stepss)):
        for i in range(10):
            ...
            walls = env1.make_road(type_)

            fig, update_m = start(env1, type_, tm,plot)

            stc = 0
            for frame in range(1000):
                crowd_count_pedestrains,crowd_pressure_density,crowd_pressure_velocity,return_ = update_m(frame) # 함수안의 함수를 들고옴 , 여기서 정보들 다 들고오기 그리고 animation 함수 가지고 저 확실한 정보틀 토대로 함수 틀 다시 짜보기 # max 값 받아오는걸로 수정하기
                if return_ == False:
                    break
