...
# 사용자 정의 모듈
import library.SF_navigation as Navigation
import library.SF_roadnetwork as RN
import library.SF_pedestrian as PD
import library.SF_agent as AGENT
...
#                 print(self.intercection_point_list, p.recommendation[0][0] , [p.target_box_index-1])
    ...
    def run_simulation(self,tm):
        ...
        for frame in tqdm(range(1000)):
            self.update(frame) # 함수안의 함수를 들고옴 , 여기서 정보들 다 들고오기 그리고 animation 함수 가지고 저 확실한 정보틀 토대로 함수 틀 다시 짜보기 # max 값 받아오는걸로 수정하기


# predict =0 train(모델로 가중치 변경) , predict =1 random (모델을 쓰지만 학습안된 모델),  predict =2 최단거리 (모델없이 가중치 길 거리로 유지하고 빠른길만)
def Trian(env,tm,plot,predict,start,set_time): # 데이터가 학습을 할때 아직 state 로 사람이 모델을 통과하지 않아 기록되지 않은 곳은 그 지점의 모델의 action 결과도 나오지 않아서 초기에 변화하는 길의 개수는 적다 추후에 모든 모델상에서 액션을 일관되게 predict 하도록 변환해야한다.
    reward_list = []
    ...
    for episode in tqdm(range(set_time)):  # 1000 에피소드 동안 학습
        for pp in env.pedestrians: # 모델이 액션을 한다고 해도 매시간마다 액션하지만 20타임마다 한번씩 생성되는 사람한테만 영향을 미칠뿐 그 영향은 모델의 전체 액션이 하필 그 사람의 경로에 크나큰 영향을 미치지 않는이상 경로추천의 길을 크게 바꾸지 않아서 결과가 이때까지 똑같이 나왔음 모델이 있든 없든 그런데 이제 모델의 액션을 매시간마다 사람들에게 반영하도록 변경하였음
...
#         print(target_models)
#         print(states)
        for state in states: # 모델 개수가 너무 작고 바뀌는 가중치도 너무 작아서 제대로된 가중치 변경이 힘들기 때문에 경로 추천이 거진 같아진다!!!
        ...
        try:
            ...
            pass
        if predict == 0:
            agent.train()  # 에피소드가 끝나면 에이전트를 학습시킴
